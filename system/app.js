...
function renderExplorer(title = "NOTARIAT_EXPLORER") {
  ...
  content.innerHTML = \
    <div style="background:#000; color:#0f0; padding:30px; border:2px solid #ffd700; font-family:monospace; min-height:400px;">
      ...
      <p>> STATUS: ONLINE | 15_MODULE_SYNC: ACTIVE</p>
      <p>> TIMESTAMP: \</p>
      <div style="margin-top:20px; border:1px solid #0f0; padding:10px;">[REINFORCED] System-Matrix stabilisiert.</div>
    </div>\;
}
...
async function loadPage(page, file = null) {
  ...
  if (page === 'notariat') { renderExplorer("MANUAL_OVERRIDE"); return; }
  if (!routes[page]) page = 'home';

  const target = file || routes[page].path;

  // Dropdown neu aufbauen und KLICKBAR machen
  dropdown.innerHTML = '';
  ...
  if (page === 'launch' && !file) {
    content.innerHTML = '<h1 style="text-align:center;">GoldenChain Launch Pad</h1>';
    return;
  }
  ...
  try {
    ...
    renderExplorer("AUTO_PROTECT: " + (target || "NULL_PATH"));
  }
}
